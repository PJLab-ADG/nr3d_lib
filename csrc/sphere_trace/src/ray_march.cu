...
__global__ void dense_grid_ray_march_prase1_kernel(uint32_t n_elements, uint32_t max_segs_per_ray,
                                                   DenseGrid grid,
    ...
    o_rays_n_segs[i] = grid.ray_march<ENABLE_DEBUG>(
        origin, dir, rays_near[i], rays_far[i], nullptr, max_segs_per_ray, o_debug_rays_flag + i,
        o_debug_rays_march_poses + i * max_segs_per_ray,
        o_debug_rays_march_voxels + i * max_segs_per_ray);
}

__global__ void dense_grid_ray_march_prase2_kernel(
    ...
    glm::vec3 space_scale = 0.5f * glm::vec3(grid.res());
    glm::vec3 origin = (rays_o[ray_idx] + 1.0f) * space_scale;
    glm::vec3 dir = rays_d[ray_idx] * space_scale;
    grid.ray_march<false>(origin, dir, rays_near[ray_idx], rays_far[ray_idx],
                          o_rays_segs + rays_segs_offset[i]);
}
...
ray_march(const DenseGrid &grid, at::Tensor rays_o, at::Tensor rays_d, at::Tensor rays_near,
          at::Tensor rays_far, bool enable_debug) {
    ...
    // Prase 1: compute number of segments for each ray
    ...
    if (enable_debug) {
        ...
        linear_kernel(dense_grid_ray_march_prase1_kernel<false>, 0, stream, n_rays,
    ...
    // Prase 2: compute ray segments
    if (total_segs)
        linear_kernel(dense_grid_ray_march_prase2_kernel, 0, stream, n_rays, grid,
    ...
    return std::make_tuple(candidate_rays_idx, pack_info, rays_segs, debug_info);
}