...
// Device primitive for a single ray-AABB intersection using Slabs method.
// This function avoids possible miss of intersections on the edges or corners of a block caused by 
// numerical error and returns the depths of entry and exit points simultaneously.
// The depth of entry is never negative, while the depth of exit can be any value in real domain.
// So a ray intersects with a block if and only if the depth of exit is larger than the depth of entry.
 static __inline__ __device__ float2 ray_aabb_in_out(
    const float3 query,  // query point (or ray origin)
    const float3 dir,    // ray direction
    const float3 invdir, // ray inverse direction
    const float3 sgn,    // sgn bits
    ...
    // Compute distance to planes
    float d0_in = fmaf(r, sgn.x, - o.x) * invdir.x;
    float d1_in = fmaf(r, sgn.y, - o.y) * invdir.y;
    float d2_in = fmaf(r, sgn.z, - o.z) * invdir.z;
    float d0_out = fmaf(r, -sgn.x, - o.x) * invdir.x;
    float d1_out = fmaf(r, -sgn.y, - o.y) * invdir.y;
    float d2_out = fmaf(r, -sgn.z, - o.z) * invdir.z;
    float d_in = fmax(fmax(d0_in, d1_in), fmax(d2_in, 0.0f));
    float d_out = fmin(fmin(d0_out, d1_out), d2_out);

    return make_float2(d_in, d_out);
}
