...
import os
import yaml
import addict # pip install addict
import argparse
...
def parse_device_ids(device_ids, is_ddp=False, to_torch=False) -> List[int]:
    if is_ddp:
    ...
    if to_torch:
        import torch
        device_ids = [torch.device(f'cuda:{did}') for did in device_ids]
    return device_ids
    ...
    def parse(
        ...
        if resume_dir is not None:
            ...
            if config_path is not None:
                ...
                if stand_alone:
                    assert 'exp_dir' in config, 'Please specify exp_dir in task_config'
                    config.exp_dir = config.exp_dir.replace('[','_').replace(']','_').replace(', ','_').replace(',','_')
