...
__all__ = [
    'opencv_distort_points', 
    'opencv_undistort_points'
]
...
from kornia.geometry.calibration.undistort import undistort_points as opencv_undistort_points

def opencv_distort_points(
    points: torch.Tensor, K: torch.Tensor, dist: torch.Tensor, 
    new_K: Optional[torch.Tensor] = None, # additional K for input undistorted points
    min_radial: float = None, max_radial: float = None, 
) -> torch.Tensor:
    ...
    Args:
        ...
        new_K: Intrinsic camera matrix of the input un-distorted image. By default, it is the same as K but you may additionally
    ...
    # NOTE: Jianfei: If the radial distortion is too large, the computed coordinates will be unreasonable (might even flip signs).
    valid = torch.ones_like(r2, dtype=torch.bool, device=r2.device)
    if min_radial is not None:
        valid = valid & (rad_poly >= min_radial)
    if max_radial is not None:
        valid = valid & (rad_poly <= max_radial)
