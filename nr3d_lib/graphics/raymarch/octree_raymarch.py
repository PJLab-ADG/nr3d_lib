...
from nr3d_lib.graphics.raymarch import RaymarchRetSingle
from nr3d_lib.graphics.raytest import octree_raytrace_fixed_from_kaolin
from nr3d_lib.graphics.pack_ops import get_pack_infos_from_boundary, get_pack_infos_from_first, get_pack_infos_from_n, interleave_linstep, octree_mark_consecutive_segments, packed_diff, merge_two_packs_sorted_a_includes_b, merge_two_packs_sorted_aligned
from nr3d_lib.graphics.raysample import batch_sample_step_linear, batch_sample_step_wrt_depth, batch_sample_step_wrt_sqrt_depth, interleave_sample_step_linear, interleave_sample_step_linear_in_packed_segments, interleave_sample_step_wrt_depth_clamped, interleave_sample_step_wrt_depth_in_packed_segments
...
def octree_raymarch_segment_step(
    ...
    if nugget_ridx.numel() > 0:
        ...
        return RaymarchRetSingle(ridx_hit, samples, depth_samples, deltas, ridx, pack_infos, sidx, seg_pack_infos)
    else:
        return RaymarchRetSingle(None, None, None, None, None, None, None, None)
...
def octree_raymarch_voxel_step(
    ...
    if nugget_ridx.numel() > 0:
        ...
        return RaymarchRetSingle(ridx_hit, samples, depth_samples, deltas, ridx, pack_infos, pidx, point_pack_infos)
    else:
        return RaymarchRetSingle(None, None, None, None, None, None, None, None)
...
def octree_raymarch_two_level(
    ...
    return RaymarchRetSingle(ret1.ridx_hit, samples, depth_samples, deltas, ridx, pack_infos)
