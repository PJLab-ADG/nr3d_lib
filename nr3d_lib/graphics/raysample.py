...
from nr3d_lib.graphics.pack_ops import get_pack_infos_from_n, interleave_arange_simple, interleave_linstep, packed_cumsum, packed_diff, packed_div, packed_invert_cdf, interleave_sample_step_wrt_depth_clamped, interleave_sample_step_wrt_depth_in_packed_segments
...
if __name__ == "__main__":
    ...
    def test_sample_step_wrt_depth_clamp(device=torch.device('cuda')):
        ...
        def sample_step_wrt_depth_clamp_min_v1(
            ...
            prefix_shape=(), dtype=torch.float, device=None, perturb=False,
    ...
    def test_interleave_sample_step_linear(device=torch.device('cuda')):
        from torch.utils.benchmark import Timer
        from nr3d_lib.graphics.pack_ops import interleave_linstep
    ...
    def test_packed_sample_cdf(device=torch.device('cuda')):
        from torch.utils.benchmark import Timer
        from nr3d_lib.graphics.nerf import packed_tau_to_vw
        from nr3d_lib.graphics.pack_ops import packed_searchsorted, interleave_linspace
        num_to_sample = 40
