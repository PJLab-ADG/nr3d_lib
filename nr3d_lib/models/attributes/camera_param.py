...
from nr3d_lib.graphics.cameras import \
...
class CameraBase(AttrNested):
    ...
    @property
    def H(self):
        # NOTE: Rounding to floor; align with the behavior of `img_to_torch_and_downscale()`
        H = torch.floor(self.subattr.H.tensor / self.scale_y).long() # Rounding to floor
        if is_scalar(H):
            return int(H.item())
        else:
            H_first = H.flatten()[0]
    ...
    def W(self):
        # NOTE: Rounding to floor; align with the behavior of `img_to_torch_and_downscale()`
        W = torch.floor(self.subattr.W.tensor / self.scale_x).long() # Rounding to floor
        if is_scalar(W):
            return int(W.item())
        else:
            W_first = W.flatten()[0]
