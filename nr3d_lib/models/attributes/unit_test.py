...
if __name__ == "__main__":
    ...
    def test_interp1d(device=torch.device('cuda')):
        from icecream import ic
        length = 7
        ts_keyframes = torch.arange(length, device=device, dtype=torch.float)
        
        # Seperately interpolate rot and trans
        transform = TransformRT(
            rot=RotationQuaternion(torch.randn(length, 4), learnable=True), 
            trans=Translation(torch.randn(length, 3), learnable=True),  
            device=device)
        t1 = transform.interp1d(ts_keyframes, [0.4])
        ic(transform[0].translation())
        ic(t1.translation())
        ic(transform[1].translation())
        ic(transform[0].translation() * 0.6 + transform[1].translation() * 0.4)

    def test_interp1d_valid(device=torch.device('cuda')):
        from icecream import ic
        length = 7
        ts_keyframes = torch.arange(length, device=device, dtype=torch.float)
        
        valid_flag = Valid(torch.rand([length,], device=device)>0.3)
        v1 = valid_flag.interp1d(ts_keyframes, [-0.9, 0.4, 3.1, 90.])
        ic(valid_flag.tensor)
        ic(v1.tensor)
