...
try:
    ...
    class TcnnNeRF(ModelMixin, nn.Module):
        def __init__(
            ...
            self.pts_blocks = TcnnFCBlockWSkips(input_ch_pts, W, D=D, W=W, skips=skips, activation=activation, output_activation='none', seed=seed).to(device)
            if self.use_view_dirs:
                # W -> 1
                self.sigma_layer = DenseLayer(W, 1, activation=sigma_activation, dtype=self.dtype, device=device)
                # W + input_ch_view -> W//2 -> W//2 -> 3
                self.rgb_blocks = TcnnFCBlock(input_ch_view + W, 3, D=2, W=W//2, activation=activation, output_activation=rgb_acitvation, seed=seed).to(device)
            else:
    ...
    class TcnnEmbeddedNeRF(ModelMixin, nn.Module):
        def __init__(
            ...
            self.pts_blocks = TcnnNetWSkips(
                ...
                D=D, W=W, skips=skips, activation=activation, output_activation='none').to(device)
            self.sigma_layer = DenseLayer(W, 1, activation=sigma_activation, dtype=self.dtype, device=device)
            ...
            if self.use_view_dirs:
                ...
                n_view_dirs = 0
            # h_appear
            if self.use_h_appear: view_cfgs.append(tcnn_encoding_config(n_appear_embedding, **appear_embed_cfg))
            ...
            self.rgb_blocks = TcnnNet(
                ...
                D=2, W=W//2, activation=activation, output_activation=rgb_acitvation, seed=seed).to(device)
        
        def forward(self, x: torch.Tensor, v: torch.Tensor=None, *, h_geo_embed: torch.Tensor=None, h_appear: torch.Tensor=None):
            if self.pos_embed_include_input:
                x = x.tile(2)
            ...
            sigma = self.sigma_layer(h)
            uses = (True, self.use_view_dirs, self.use_h_appear)
            rgb = self.rgb_blocks(torch.cat([i for idx,i in enumerate((h, v, h_appear)) if uses[idx]], dim=-1))
            return dict(rgb=rgb, sigma=sigma.squeeze(-1))   
        def forward_density(self, x: torch.Tensor, *, h_geo_embed: torch.Tensor=None):
            if self.pos_embed_include_input:
                x = x.tile(2)
            h = self.pts_blocks(x if not self.use_geo_embedding else torch.cat([x, h_geo_embed], dim=-1))
            sigma = self.sigma_layer(h)
    ...
    class TcnnRadianceNet(ModelMixin, tcnn.NetworkWithInputEncoding):
        ...
        def __init__(
            ...
            in_features = (3 if self.use_pos else 0) + (3 if self.use_view_dirs else 0) + (3 if self.use_nablas else 0) + n_extra_feat + n_appear_embedding
            ...
            # h_extra
            if self.use_extra_feat: cfgs.append(tcnn_encoding_config(n_extra_feat, **extra_feat_embed_cfg))
            
            # h_appear
            if self.use_h_appear: cfgs.append(tcnn_encoding_config(n_appear_embedding, **appear_embed_cfg))
            
            encoding_config = {"otype": "Composite", "nested": cfgs} if len(cfgs) > 1 else cfgs[0]
            # [x, v, n, h_extra, h_appear]
            self._uses = (self.use_pos, self.use_view_dirs, self.use_nablas, self.use_extra_feat, self.use_h_appear)
        ...
        @profile
        def forward(self, x: torch.Tensor, v: torch.Tensor=None, n: torch.Tensor=None, *, h_extra: torch.Tensor=None, h_appear: torch.Tensor=None) -> torch.Tensor:
            prefix = x.shape[:-1]
            uses = self._uses
            ...
            if self.use_view_dirs and self.dir_embed_include_input:
                v = v.tile(2)
            h = torch.cat([i for idx, i in enumerate((x, v, n, h_extra, h_appear)) if uses[idx]], dim=-1)
            return dict(rgb=super().forward(h.flatten(0, -2)).unflatten(0, prefix))
