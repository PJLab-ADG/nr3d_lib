...
class LoTDNeuS(ModelMixin, nn.Module):
    ...
    def populate(self, bounding_size=None, aabb=None, device=None):
        ...
        self.implicit_surface = LoTDSDF(**self.surface_cfg, dtype=self.dtype, device=device)
    ...
    def training_before_per_step(self, cur_it: int, logger: Logger = None):
        ...
        self.radiance_net.training_before_per_step(cur_it, logger=logger)
    ...
    def query_sdf(self, x: torch.Tensor, input_normalized=True) -> torch.Tensor:
        return self.forward_sdf(x, return_h=False, input_normalized=input_normalized)['sdf']
    ...
    def forward(
        self, x: torch.Tensor, *, 
        v: torch.Tensor = None, h_appear: torch.Tensor = None, 
    ...
    def forward_in_obj(
        ...
        if with_rgb:
            rgb = torch.full([*x.shape[:-1], 3], invalid_color, dtype=torch.float, device=self.device)
            if valid_i[0].numel() > 0:
                raw_ret.update(self.radiance_net(
                    x[valid_i], 
                    v=v.expand(*prefix, 3)[valid_i] if self.use_view_dirs else None, 
                    n=raw_ret['nablas'].detach().clamp(-1,1) if self.use_nablas else None, 
                    h_extra=raw_ret['h'], h_appear=h_appear))
                rgb.index_put_(valid_i, raw_ret['rgb'])
            ret.update(rgb=rgb)
    ...
    def training_setup(self, training_cfg: Union[Number, dict], name_prefix: str = ''):
        ...
        if (self.ctrl_var is None) \
            ...
            """
            NOTE: betas=(0.9,0.99) is much better & more stable than betas=(0.9,0.999), 
            Explain:
            - Smaller `beta2` values (smaller exponential decay rate for the second moment estimates) \
                significantly stabilize the convergence of the Eikonal loss.\
                Which means we do not want too much information from the last step;
            """
            pg_network = {
    ...
    def training_initialize(self, config: dict, logger: Logger=None, log_prefix: str=None):
...
if __name__ == "__main__":
    def unit_test(device=torch.device('cuda')):
        ...
        out = m.forward(x, v)
        print(out['sdf'].shape, out['rgb'].shape)
