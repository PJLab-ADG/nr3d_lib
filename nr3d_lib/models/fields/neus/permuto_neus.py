"""
@file   permuto_neus.py
@author Jianfei Guo, Shanghai AI Lab
@brief  NeuS network characterized using the Permutohedral-encoding model.
"""
...
from nr3d_lib.models.fields.sdf import PermutoSDF
...
class PermutoNeuS(ModelMixin, nn.Module):
    def __init__(
        self,
        surface_cfg: dict,
    ...
    def populate(self, bounding_size=None, aabb=None, device=None):
        ...
        self.implicit_surface = PermutoSDF(**self.surface_cfg, n_input_dim=3, dtype=self.dtype, device=device)
    ...
    def forward_in_obj(
        ...
        valid_i = ((net_x > -1) & (net_x < 1)).all(dim=-1).nonzero(as_tuple=True)
...
class PermutoNeuSModel(NeusRendererMixin, PermutoNeuS):
    """
    MRO:
    -> PermutoNeuSModel
    -> NeusRendererMixin
