...
class VarSingleLearned(nn.Module):
    def __init__(self, ln_inv_s_init: float, ln_inv_s_factor: float = 10.0, device=None) -> None:
        ...
        self.ln_inv_s = nn.Parameter(data=torch.tensor([self.ln_inv_s_init], device=device, dtype=torch.float), requires_grad=True)
    def set_iter(self, it: int):
        self.it = it
...
class VarSingleManual(nn.Module):
    def __init__(self, inv_s_anneal_cfg: dict, device=None) -> None:
        super().__init__()
...
class VarSingleLnManual(nn.Module):
    def __init__(self, ln_inv_s_anneal_cfg: dict, ln_inv_s_factor: float = 10.0, device=None) -> None:
        super().__init__()
        self.ln_inv_s_factor = ln_inv_s_factor
