...
from nr3d_lib.models.blocks import get_blocks, MLP
...
class LoTDSDF(ModelMixin, nn.Module):
    def __init__(
        ...
        self.encoding_cfg = encoding_cfg = deepcopy(encoding_cfg)
        ...
        self.encoding = LoTDEncoding(3, **self.encoding_cfg, dtype=self.dtype, device=device)
        ...
        if self.is_extra_feat_from_output:
            ...
            self.decoder, self.decoder_type = get_multires_decoder(self.encoding.lod_meta.level_n_feats, (1+self.n_extra_feat_from_output), n_extra_embed_ch=self.n_extra_embed, **self.decoder_cfg, dtype=self.dtype, device=device)
        else:
            self.n_extra_feat = self.encoding.out_features
            self.decoder, self.decoder_type = get_multires_decoder(self.encoding.lod_meta.level_n_feats, 1, n_extra_embed_ch=self.n_extra_embed ,**self.decoder_cfg, dtype=self.dtype, device=device)
        ...
        if self.geo_init_method == 'geometric' or self.geo_init_method == 'pretrain_after_geometric':
            ...
            if self.encoding.annealer is not None:
                ...
                start_n_feats = sum(self.encoding.lod_meta.level_n_feats[:start_level+1])
            else:
                start_level = 0
                start_n_feats = 0
            with torch.no_grad():
                # for l in range(start_level, self.encoding.lod_meta.n_levels, 1):
                ...
                elif isinstance(self.decoder, DenseLayer):
                    nn.init.zeros_(self.decoder.weight[:, start_n_feats:])
                elif isinstance(self.decoder, MLP):
                    nn.init.zeros_(self.decoder.layers[0].weight[:, start_n_feats:])
                else:
                    if self.decoder_cfg['use_tcnn_backend']:
                        from nr3d_lib.models.tcnn_adapter import TcnnFCBlock
                        if isinstance(self.decoder, TcnnFCBlock):
                            # Set the first layer's weight to zero
                            nn.init.zeros_(self.decoder.params[:self.decoder.in_features * self.decoder_cfg["W"]])
                    else:
                        raise NotImplementedError(f"zero_out does not support {type(self.decoder)} decoder")
    ...
    def forward_sdf_nablas(
        ...
        max_level: int=None, grad_guard=None, return_net_nablas=False) -> Dict[str, torch.Tensor]:
        ...
        with torch.enable_grad():
            h, dy_dx = self.encoding.forward_dydx(x, max_level=max_level, need_dL_dinput=need_dL_dinput)
            if not self.use_extra_embed:
                h_full = h
            else:
                h_embed = self.extra_embed_fn(x)
        ...
        if not self.use_extra_embed:
            ...
            #---- Encoding bwd_input
            nablas = self.encoding.backward_dydx(dL_dh_full[..., :h.size(-1)], dy_dx, x, max_level=max_level, grad_guard=grad_guard)
        ...
        x = x.requires_grad_(need_dL_dinput)
        ...
        ret = dict(sdf=sdf, h=h, nablas=(nablas * (self.sdf_scale / self.space.radius3d_original)))
        if return_net_nablas:
            ret["nablas_normalized"] = nablas
        return ret
