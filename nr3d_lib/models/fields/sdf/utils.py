...
from nr3d_lib.models.blocks import MLP
from nr3d_lib.models.loss.safe import safe_mse_loss
from nr3d_lib.utils import check_to_torch, tensor_statistics

@torch.no_grad()
def idr_geometric_init(decoder: MLP, *, radius_init: float, n_embed: int, inside_out=False):
...
def pretrain_sdf_sphere(
    ...
    # Shape configs
    target_radius: float=0.5, # In (uni-scaled) obj coords
    target_radius_in_net: float=None, # In network coords
    target_origin: Union[np.ndarray, torch.Tensor]=None, # In (uni-scaled) obj coords
    inside_out=False, 
...
def pretrain_sdf_capsule(
    ...
    surface_distance_in_net: float = None, # In net coords space
    inside_out=False, 
    # Debug & logging related
    logger: Logger=None, log_prefix: str=None, debug_param_detail=False):
    """
    ...
    with torch.enable_grad():
        with tqdm(range(num_iters), desc=f"=> Pretraining SDF...") as pbar:
            for it in pbar:
                ...
                if surface_distance_in_net is None:
                    sdf_gt_in_obj = surface_distance - min_dis_in_obj
                    # Convert SDF GT value in real-world obj's unit to network's unit
                    sdf_gt = sdf_gt_in_obj / implicit_surface.sdf_scale
                else:
                    sdf_gt = surface_distance_in_net - min_dis_in_obj / implicit_surface.sdf_scale
