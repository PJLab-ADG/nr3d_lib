...
from nr3d_lib.models.accelerations import get_accel, accel_types_batched_dynamic_t
...
class NeusRendererMixinBatchedDynamic(ModelMixin):
    """
    ...
    def __init__(
        ...
        **net_kwargs,
        ...
        mro = type(self).mro()
        super_class = mro[mro.index(NeusRendererMixinBatchedDynamic)+1]
    ...
    def populate(self, *args, **kwargs):
        ...
        self.accel: accel_types_batched_dynamic_t = None if self.accel_cfg is None \
            else get_accel(space=self.space, device=self.device, **self.accel_cfg)
        self.upsample_s_divisor = 1.0

    # @property
    # def space(self) -> BatchedDynamicSpace:
    #     return super().space
    ...
    def sample_pts_uniform(self, num_samples: int):
        ...
        ret = self.forward_sdf_nablas(x, bidx=bidx, ts=ts, skip_accel=True)
    ...
    def sample_pts_in_occupied(self, num_samples: int):
        ...
        x, bidx, ts = self.accel.cur_batch__sample_pts_in_occupied(num_samples)
        ret = self.forward_sdf_nablas(x, bidx=bidx, ts=ts, skip_accel=True) # Do not upsate_samples here (usally there are too less samples here.)
        ret = {k: v.to(x.dtype) for k, v in ret.items()}
        ret['net_x'] = x # NOTE: in network's uniformed space; not in world space.
        return ret

    def set_condition(self, *args, ins_inds_per_batch: torch.Tensor=None, skip_accel=False, **kwargs):
    ...
    def query_sdf(self, x: torch.Tensor, bidx: torch.LongTensor, ts: torch.Tensor, **kwargs):
        return super().query_sdf(x, bidx=bidx, ts=ts, **kwargs)

    def forward_sdf(self, x: torch.Tensor, bidx: torch.LongTensor, ts: torch.Tensor,
                    skip_accel=False, **kwargs):
        ret = super().forward_sdf(x, bidx=bidx, ts=ts, **kwargs)
        if self.training and (not skip_accel) and (self.accel is not None):
            self.accel.cur_batch__collect_samples(x, bidx=bidx, ts=ts, val=ret['sdf'].data)
        return ret
    
    def forward_sdf_nablas(self, x: torch.Tensor, bidx: torch.LongTensor, ts: torch.Tensor, 
                           skip_accel=False, **kwargs):
        ret = super().forward_sdf_nablas(x, bidx=bidx, ts=ts, **kwargs)
        if self.training and (not skip_accel) and (self.accel is not None):
            self.accel.cur_batch__collect_samples(x, bidx=bidx, ts=ts, val=ret['sdf'].data)
        return ret
