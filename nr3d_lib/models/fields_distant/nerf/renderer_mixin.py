...
from nr3d_lib.graphics.pack_ops import get_pack_infos_from_n, packed_div, packed_sum
from nr3d_lib.graphics.nerf import packed_alpha_to_vw, ray_alpha_to_vw, tau_to_alpha, packed_volume_render_compression
...
class NeRFRendererMixinDistant(ModelMixin):
    def __init__(
        self, 
        ...
        **net_kwargs
        ...
        mro = type(self).mro()
        super_class = mro[mro.index(NeRFRendererMixinDistant)+1]
    ...
    def populate(self, *args, **kwargs):
        ...
        self.accel = None if self.accel_cfg is None \
    ...
    def _ray_query_march(
        self, ray_tested: Dict[str, torch.Tensor], *, 
        with_rgb=True, with_normal=False, perturb=False, march_cfg=dict(), 
        ...
        if compression:
            #----------------
            # Compute compact samples based on rendered visibility
            with torch.no_grad():
                ...
                if bypass_sigma_fn is not None:
                    ...
                    sigmas = self.forward_density(raymarch_ret.samples)['sigma']
                    alphas = tau_to_alpha(sigmas * raymarch_ret.deltas)
            ...
            raymarch_ret = named_tuple_nerfpp_raymarch_ret(
                ...
                raymarch_ret.samples[pidx_useful], 
                raymarch_ret.depth_samples[pidx_useful], 
                raymarch_ret.deltas[pidx_useful], 
                raymarch_ret.ridx[pidx_useful], 
                pack_infos_hit_useful
            )
        else:
            pass
        ...
        volume_buffer = dict(
            type='packed', 
            rays_inds_hit=rays_inds[raymarch_ret.ridx_hit], 
        ...
        if with_rgb:
            ...
            # Get embedding code
            h_appear = ray_tested['rays_h_appear'][raymarch_ret.ridx]\
                if ray_tested.get('rays_h_appear', None) is not None else None
            
            net_out = self.forward(x=raymarch_ret.samples, v=view_dirs[raymarch_ret.ridx], h_appear=h_appear)
            volume_buffer["rgb"] = net_out["rgb"].to(dtype)
        else:
            net_out = self.forward_density(raymarch_ret.samples)
    ...
    def ray_query(
        ...
        #----------------
        if query_mode == 'march':
            volume_buffer, query_details = self._ray_query_march(
...
if __name__ == "__main__":
    def test_r_inveser_sampling(device=torch.device('cuda')):
        import numpy as np
    ...
    def test_r_log_sampling(device=torch.device('cuda')):
        import numpy as np
        import matplotlib.pyplot as plt
        import seaborn as sns
    ...
    # def test_r_inverse_sampling_with_k(device=torch.device('cuda')):
    #     import numpy as np
    #     import matplotlib.pyplot as plt
    #     import seaborn as sns
