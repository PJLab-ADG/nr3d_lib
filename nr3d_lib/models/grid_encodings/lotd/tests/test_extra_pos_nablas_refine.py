...
def fun1(x, opt=1, need_dL_dinput=True):
    """
    ...
    h, dy_dx = enc.forward_dydx(x, need_dL_dinput=need_dL_dinput)
    # NOTE: All is correct
    if opt == 1:
...
def fun2(x, opt: int = 1, need_dL_dinput=True):
    """
    2 autograd call
    """
    for n, p in enc.named_parameters():
...
nablas1_1, nablas_extra1_1 = fun1(x, opt=1, need_dL_dinput=True)
...
nablas1_1_false, nablas_extra1_1_false = fun1(x, opt=1, need_dL_dinput=False)
