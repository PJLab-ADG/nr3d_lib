...
class ProfileNode:
    ...
    def close(self, sync: bool=False) -> "ProfileNode":
        if self.closed:
            raise RuntimeError("The node has been closed")
        self._device_end_event.record()
        if sync:
            self._device_end_event.synchronize()
...
class StatisticData:
    ...
    def get_report(self, title: str = None, sort_by: str = None, decent: str = True):
        ...
        flatten_data = self.get_raw_data(sort_by=sort_by, decent=decent)
    ...
    def __enter__(self, *args, **kwargs):
        if default_profiler is not None:
            ...
            if self.fn is not None:
                if self.fn_sig is None:
                    self.fn_sig = inspect.signature(self.fn)
                bound_args = self.fn_sig.bind(*args, **kwargs)
                bound_args.arguments.update(self=self.fn.__qualname__)
                name = name.format(**bound_args.arguments)
...
class _DebugProfileWrap:
    ...
    def __exit__(self, *args, **kwargs):
        ...
        print(f"{self.name}: host duration {self.node.host_duration:.3f}ms, "
              f"device duration {self.node.device_duration:.3f}ms")
